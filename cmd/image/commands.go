...
func Command() *cobra.Command {
	...
	list := &cobra.Command{Use: "list", Aliases: []string{"ls"}, Short: "List images", RunE: List}
	cliutil.AddFormatFlag(list)
	inspect := &cobra.Command{Use: "inspect REF", Short: "Show one image (JSON)", Args: cobra.ExactArgs(1), RunE: Inspect}
	rm := &cobra.Command{Use: "rm REF [REF...]", Short: "Remove image(s)", Args: cobra.MinimumNArgs(1), RunE: RM}
